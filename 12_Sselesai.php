...
if (isset($_GET['kode_order'])){
    ...
    $sql = "UPDATE `order` SET `status` = 'selesai' WHERE kode_order = '$kode_order' ";
    $hasil = mysqli_query($koneksi, $sql);
    echo "<script> confirm('Apakah Order Ini telah selesai?') </script>";
...
if ($_SESSION['level'] == 1){
    ...
    $previous = $halaman - 1;
    $next = $halaman + 1;
    $data = mysqli_query($koneksi, "SELECT * FROM `order` WHERE status = 'selesai'");
    $jumlah_data = mysqli_num_rows($data);
    $total_halaman = ceil($jumlah_data / $batas);
    $hasil = mysqli_query($koneksi,  "SELECT * FROM `order` WHERE status = 'selesai' limit $halaman_awal, $batas");
    $no = $halaman_awal+1;
    ...
    $jumlah_data = mysqli_num_rows($data);
    $total_halaman = ceil($jumlah_data / $batas);
    $hasil = mysqli_query($koneksi,  "SELECT * FROM `order` WHERE status = 'selesai' ORDER BY $orderBy LIMIT $halaman_awal, $batas");
    $no = $halaman_awal + 1;

    // SEARCH
    if (isset($_GET['cari'])){
        $cari  = $_GET['cari'];
        $sql_cari  = "SELECT * FROM `order` WHERE status = 'selesai' AND 
        ...
        if (mysqli_num_rows($hasil) > 0){
            ...
            echo "<script> window.location.href='12_Sselesai.php' </script>";
        }
    }
}else{
    ...
    $previous = $halaman - 1;
    $next = $halaman + 1;
    $data = mysqli_query($koneksi, "SELECT * FROM `order` WHERE status = 'selesai' AND pembeli = '$username'");
    $jumlah_data = mysqli_num_rows($data);
    $total_halaman = ceil($jumlah_data / $batas);
    $hasil = mysqli_query($koneksi,  "SELECT * FROM `order` WHERE status = 'selesai' AND pembeli = '$username' limit $halaman_awal, $batas");
    $no = $halaman_awal+1;
    ...
    $jumlah_data = mysqli_num_rows($data);
    $total_halaman = ceil($jumlah_data / $batas);
    $hasil = mysqli_query($koneksi,  "SELECT * FROM `order` WHERE status = 'selesai' AND pembeli = '$username' ORDER BY $orderBy LIMIT $halaman_awal, $batas");
    $no = $halaman_awal + 1;

    // SEARCH
    if (isset($_GET['cari'])){
        $cari  = $_GET['cari'];
        $sql_cari  = "SELECT * FROM `order` WHERE status = 'selesai' AND pembeli = '$username' AND 
                (kode_order LIKE '%$cari%' OR tanggal LIKE '%$cari%' OR 
                jam LIKE '%$cari%' OR pelayan LIKE '%".$cari."%' OR no_meja LIKE '%$cari%')";
        $hasil = mysqli_query($koneksi, $sql_cari);
            ...
            <?php
                ...
                while ($row = mysqli_fetch_assoc($hasil)) {
                    echo "<tr>
                            ...
                            <td><a href='5_dataOrder.php?kode_order=" . $row['kode_order'] . "' onclick='return confirm(\"Anda yakin akan menghapus Data Order ini?\")'><button type='button' class='btn btn-danger'>Hapus</button></a>
                            <a href='6_orderDetil.php?kode_order=" . $row['kode_order'] . "'><button type='button' class='btn btn-success'>Detil </button></a></td>
                        </tr>";
                    $no++;
                }
            ?>
